...
export default function SegmentKohSamui() {
    return (
        <StaticQuery
            query={graphql`
                query {
                    allSanityVilla(limit: 4, sort: {fields: villaName}, filter: {villaLocation: {eq: "Koh Samui"}, featuredHome: {eq: true}}) {
                        edges {
                            node {
                                villaName
                                villaHeadline
            ...
            render={data => (
                <div>
                    <h2 className="text-4xl font-extrabold tracking-tight">
                        <Link to="/">Villas in Koh Samui{' '}
                            <span className="text-base font-normal tracking-normal">Click here to see all our Koh Samui villas</span>
                        </Link>
                    </h2>
