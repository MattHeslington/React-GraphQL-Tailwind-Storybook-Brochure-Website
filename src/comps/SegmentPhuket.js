...
export default function SegmentPhuket() {
    return (
        <StaticQuery
            query={graphql`
                query {
                    allSanityVilla(limit: 4, sort: {fields: villaName}, filter: {villaLocation: {eq: "Phuket"}, featuredHome: {eq: true}}) {
                        edges {
                            node {
                                villaName
                                villaHeadline
            ...
            render={data => (
                <div>
                    <h2 className="text-4xl font-extrabold tracking-tight">
                        <Link to="/">Villas in Phuket
                            <span className="text-base font-normal tracking-normal">Click here to see all our Phuket villas</span>
                        </Link>
                    </h2>
