...
export default function SegmentSriLanka() {
    return (
        <StaticQuery
            query={graphql`
                query {
                    allSanityVilla(limit: 4, sort: {fields: villaName}, filter: {villaLocation: {eq: "Sri Lanka"}, featuredHome: {eq: true}}) {
                        edges {
                            node {
                                villaName
                                villaHeadline
            ...
            render={data => (
                <div>
                    <h2 className="text-4xl font-extrabold tracking-tight">
                        <Link to="/">Villas in Sri Lanka
                            <span className="text-base font-normal tracking-normal">Click here to see all our Sri Lanka villas</span>
                        </Link>
                    </h2>
